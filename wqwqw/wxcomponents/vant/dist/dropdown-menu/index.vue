...
<uni-shadow-root class="vant-dist-dropdown-menu-index"><view class="van-dropdown-menu van-dropdown-menu--top-bottom custom-class">
  <view v-for="(item,index) in (itemListData)" :key="item.index" :data-index="index" :class="utils.bem('dropdown-menu__item', { disabled: item.disabled })" :style="'background-color:'+(item.options[item.value].color)" @click="onTitleTap">
...
VantComponent({
    ...
    methods: {
        updateItemListData() {
			...
			// 打印 itemListData
			    console.log('itemListData:', this.data.itemListData);
        },
        updateChildrenData() {
