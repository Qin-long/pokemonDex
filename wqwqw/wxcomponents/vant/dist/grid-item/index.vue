...
<uni-shadow-root class="vant-dist-grid-item-index"><view :class="'custom-class '+(utils.bem('grid-item', { square }))" :style="computed.wrapperStyle({ square, gutter, columnNum, index })" @click="onClick">
  <view :class="'bg-Color content-class '+(utils.bem('grid-item__content', [direction, { center, square, reverse, clickable, surround: border && gutter }]))+' '+(border ? 'van-hairline--surround' : '')" :style="' background-color:'+(color)+';'+(computed.contentStyle({ square, gutter }))">
    <block v-if="useSlot">
      <slot></slot>
    </block>
...
.van-grid-item__text {
	...
	color: var(--grid-item-text-color, #FFF);
